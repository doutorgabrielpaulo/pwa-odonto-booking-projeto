...
def generate_time_slots(date, all_unavailable):
    ...
    def is_slot_occupied(start, end):
        for res_start, res_end in all_unavailable: # Usar a lista combinada de horários indisponíveis
    ...
    for start, end in base_slots_2h30:
        slots['2h30'].append({
            ...
            "status": "occupied" if is_slot_occupied(start, end) else "available"
        })
...
@check_contract
def rent_room():
    # MODIFICADO: A rota agora passa as datas para o template
    ...
    return render_template('rent_room.html', rooms=rooms, selected_date=selected_date, prev_date=prev_date, next_date=next_date)
...
def book_room():
    ...
    # Lógica para verificar a disponibilidade e criar a reserva
    # (Exemplo: poderia verificar se já existe uma reserva para o horário)
    ...
    db.session.commit()
    log_event("Reserva de Sala", "SUCCESS", {"room_id": room_id, "date": date_str}, user_id=current_user.id, ip_address=request.remote_addr)
...
def check_tutorials():
    ...
    questions = []
    if not autoclave_pref and room.autoclave_tutorial_url:
        questions.append({'id': 'autoclave', 'name': 'Autoclave', 'url': room.autoclave_tutorial_url})
    
    if not plast_pref and room.plastificadora_tutorial_url:
        questions.append({'id': 'plastificadora', 'name': 'Plastificadora', 'url': room.plastificadora_tutorial_url})
...
def get_room_slots():
    ...
    try:
        ...
        # Combinar reservas e bloqueios
        reserved_times = {(res.start_time, res.end_time) for res in reservations_for_room}
        locked_times = {(lock.start_time, lock.end_time) for lock in temp_locks}
        all_unavailable = reserved_times.union(locked_times)
